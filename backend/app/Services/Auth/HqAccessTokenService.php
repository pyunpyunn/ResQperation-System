...
class HqAccessTokenService
{
    ...
    public function resolveFromRequest(Request $request): ?array
    {
        ...
        $userId = isset($payload->sub) ? (string) $payload->sub : null;
        ...
        if ((string) $tokenable->getKey() !== $userId || $tokenable->roleKey() !== $role || ! $tokenable->is_active) {
            return null;
        }

        if (! $accessToken->can(self::WEB_ABILITY) || ! $accessToken->can('role:'.$tokenable->roleKey())) {
            return null;
        }
